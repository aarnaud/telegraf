...
type Config struct {
	...
	// Output as text or protobuf
	PrometheusRemoteWriteFormat bool `toml:"prometheus_remote_write_format"`
}
...
func NewPrometheusSerializer(config *Config) (Serializer, error) {
	...
	if config.PrometheusExportTimestamp || config.PrometheusRemoteWriteFormat {
	...
	format := prometheus.FormatText
	if config.PrometheusRemoteWriteFormat {
		format = prometheus.FormatRemoteWrite
	}
